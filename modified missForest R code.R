...
missForestnew <- function(xmis, maxiter = 10, ntree = 100, variablewise = FALSE,
                          decreasing = FALSE, verbose = FALSE,
                          mtry = floor(sqrt(ncol(xmis))), replace = TRUE,
                          classwt = NULL, cutoff = NULL, strata = NULL,sortorder=NULL,
                          sampsize = NULL, nodesize = NULL, maxnodes = NULL,testvec=NULL,
...
{ ## ----------------------------------------------------------------------
  ...
  ## testvec      = vector indicating row indices of test set. 
  ##
  ##sortorder = vector of column indices to order by in the data (NM)
  ## ----------------------------------------------------------------------
  ...
  if (!is.null(sortorder))
    stopifnot(length(sortorder) == ncol(xmis))
  ...
  for (t.co in 1:p){
    if (is.null(xAttrib[[t.co]])){
      ...
      ximptest[is.na(xmistest[,t.co]),t.co]<-mean(xmis[,t.co], na.rm = TRUE)
    } else {
      varType[t.co] <- 'factor'
      ...
      if (class.assign != "NA's"){
        ...
        ximptest[is.na(xmistest[,t.co]),t.co] <- class.assign
      } else {
  ...
  testNAloc<-is.na(xmistest)
  allNAloc<-is.na(allxmis)
  noNAvar <- apply(allNAloc, 2, sum) # how many are missing in the vars
  #try sorting by mutual information with class
    ...
    if (parallelize=="variables"){
        ...
        results <- foreach(varInd=idx, .packages='randomForest') %cols% {
          ...
          if (typeY == 'numeric'){
            ...
            if (length(summarY) == 1){ ## if there is only one level left
              ...
              if(nrow(misXtest)>0){
                mistestY <- factor(rep(names(summarY), length(misitest)))
              }
              
            } else {
              #print("categorical RF")
      ...
      for (s in 1:p) {
        ...
        if (noNAvar[[varInd]] != 0) {
          ...
          if(nrow(misXtest)>0){
            ximptest[misitest,varInd]<-mistestY}
          #update test set imputation also. 
        }
      }
    }
    cat('done!\n')
  ...
  if (iter == maxiter){
    if (any(is.na(xtrue))){
      out <- list(ximp = Ximp[[iter]], OOBerror = OOBerr,ximptest=Ximptest[[iter]])
    } else {
      out <- list(ximp = Ximp[[iter]], OOBerror = OOBerr, error = err,ximptest=Ximptest[[iter]])
    }
  } else {
    if (any(is.na(xtrue))){
      out <- list(ximp = Ximp[[iter-1]], OOBerror = OOBerrOld,ximptest=Ximptest[[iter-1]])
    } else {
      out <- list(ximp = Ximp[[iter-1]], OOBerror = OOBerrOld,
                  error = suppressWarnings(mixError(Ximp[[iter-1]], xmis, xtrue)),ximptest=Ximptest[[iter-1]])
    }
  }
  
  class(out) <- 'missForestNew'
  return(out)
